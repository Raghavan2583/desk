...
## D066 | Split ABANDONED into DORMANT vs ARCHIVED | 2026-07-05
Found reviewing jinja2's CRITICAL score: `stg_github_maintainers.sql`'s CASE WHEN labeled both `is_archived=TRUE` (GitHub-confirmed) and `days_since_last_commit > 365` (inferred) as the same word, "ABANDONED" — overclaiming certainty for the inferred case. Jinja2 genuinely hasn't committed in 386 days but is stable/feature-complete, not abandoned; the label made a false claim look confirmed. Split into DORMANT (inferred, 365+ days, not archived) and ARCHIVED (is_archived=TRUE, GitHub-confirmed). Scoring math unchanged — this only affects the display label, not the numeric risk score, which already read `days_since_last_commit`/`is_archived` directly.
## D067 | Fix Score Modal Weight Display | 2026-07-05
`RiskScoreCard.jsx`'s SCORE_FACTORS showed CVE 50% / Maintainer 20% / Depth 20% / Trend 10% — never matched the real formula (maintainer 40% / CVE 30% / depth 20% / trend 10%, per D012, verified against `risk_score.py`'s actual `0.4/0.3/0.2/0.1` multipliers). Found while fixing D066. The point values and progress bars were always correct (pulled live from the `components` object) — only the displayed percentage labels and factor order were wrong. Corrected both.
## D068 | Isolate Ingestion Sources into Separate Jobs; Fix Downloads Honesty | 2026-07-07
6 July: pypistats.org 429'd 100% of requests for 2 hours straight; `pypi_ingest.py`'s single-job design meant the 120-min job timeout cancelled GitHub/OSV/deps.dev/dbt/scoring/export/deploy along with it — zero output for the day, none of it pypistats' fault. Split `daily_refresh.yml` into 6 jobs: `metadata` (PyPI JSON only, no pypistats) seeds the queue and exports Parquet artifacts; `downloads`/`github`/`osv`/`deps_dev` fan out in parallel (`continue-on-error: true`, own timeouts, own Parquet exports); `transform_and_deploy` hydrates from whatever artifacts exist and proceeds once `metadata` alone has succeeded. New `pypi_downloads_ingest.py` (split from `pypi_ingest.py`) adds a `CircuitBreaker` (`backoff.py`) that gives up after 20 consecutive failures instead of paying the full retry ladder for all 1,000 packages. Also fixed: 4 places (`risk_score.py` x2, `ingestion/db.py`, `RiskScoreCard.jsx`) still described the D061 "~40/day rotation" retired by D064; and the frontend showed CARRIED_FORWARD download counts identically to live ones — pill is now amber with "(previous)" inline, not hover-only.
